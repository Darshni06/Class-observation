...
### As Teacher
Everything in the teacher portal is scoped to **the teacher's assigned
class** — if two teachers share a class, they see exactly the same feedback,
progress, and reports.
- **My Feedback** — one tab per published observation date, with every scored
  parameter, its level, and the observer's remark, plus the positive points
  and areas-to-improve notes.
- **My Progress** — average score per category as progress bars, plus a score
  history table over time.
- **My Reports** — any AI report an admin has shared, viewable in full and
  exportable to Word.
